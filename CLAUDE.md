...
title falls back to bold Roboto rather than failing. It is 77 px, in an 84 px strip above the panel (`BRAND_H`), set 26 px up so the tail of the Q clears the panel. `rowAt` and `listHeight` read `BRAND_H`, so resizing the title moves the rows with it. `MenuPreviewTest` loads it too, so
